...
public class ItemBCD extends ItemArmor {
	...
	public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer)
    {
		...
		if(par3EntityPlayer.isSneaking()){
			...
			if(tank != null){
				...
				bcd.getTagCompound().setInteger(tankID, 0);
	...
	public void changeDo(ItemStack par1ItemStack, EntityPlayer player, boolean increase, Side side)
	{
		...
		if(tank == null){return;}
		confirmTagCompound(par1ItemStack);
		//System.out.println("@@@@@@@@@@@@@@@@@ AlgaeCraft has officially communicated a keypress to a server.");
		int reqAir = 0;
		if(increase){
			...
			if(tank.getItemDamage() + reqAir > tank.getMaxDamage()){
				player.addChatMessage(new ChatComponentText("Not enough air to change state"));
				return;}
			...
			player.addChatMessage(new ChatComponentText("BCD State: "+s+" Buoyancy"));
			}
		}
		else{
	...
	private static void setTankType(ItemStack stack, Item item){
		int i = item == null ? 0 : 
			(item == ACItems.itemAirTankSmall ? 1 : 0);
